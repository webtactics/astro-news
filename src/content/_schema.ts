...
export const articleSchema = (image: ImageFunction) => {
  return z.object({
    ...
    is_in_news_ticker: z.boolean().default(false),
    is_in_headline: z.boolean().default(false),
    title: z.string({ message: "Title is required" }),
    description: z.string({ message: "Description is required" }),
    cover_image: image(),
    cover_image_alt: z.string({ message: "Image alt text is required" }),
    published_time: z
      .string({ message: "Published time is required" })
      .or(z.date({ message: "Published time is required" }))
    ...
    tags: z
      ...
      .max(4, { message: "At most four tags are allowed" }),
...
export const authorSchema = (image: ImageFunction) => {
  return z.object({
    ...
    avatar: image().refine((image) => image.width > 400 && image.width < 2200, {
